//! Bridge between the scene model and `yomika-psd`.
//!
//! Walks the current scene, resolves every blob reference, and feeds the
//! result into `yomika_psd::write_document` one page at a time.
...
/// Resolved page artifacts ready to hand to `yomika-psd`.
struct ResolvedPage {
    doc: PsdDocument,
    source: DynamicImage,
...
pub fn psd_bytes_for_page(
    ...
    renderer: &yomika_app::renderer::Renderer,
...
fn text_to_psd(
    node_id: &yomika_core::NodeId,
    transform: &yomika_core::Transform,
    text: &TextData,
...
#[cfg(test)]
mod tests {
    use yomika_core::{BlobRef, NodeId, TextData, Transform};
