//! MCP server exposing Yomika operations as tools.
//!
...
//!
//! **Tools exposed:**
//!   - `yomika.apply` — apply an `Op` to the active scene
//!   - `yomika.undo` / `yomika.redo`
//!   - `yomika.open_project` / `yomika.close_project`
//!   - `yomika.start_pipeline` — kick off a pipeline run
//!
...
use yomika_core::{NodeId, Op, PageId, ReadingOrder};

use crate::AppState;

/// Server state handed to each tool call. Carries the shared `App`.
#[derive(Clone)]
pub struct YomikaServer {
    state: AppState,
}
...
#[tool_router]
impl YomikaServer {
    #[tool(name = "yomika.apply", description = "Apply an Op to the active scene")]
    async fn apply(
        &self,
    ...
    #[tool(name = "yomika.undo", description = "Undo the most recent op")]
    ...
    #[tool(name = "yomika.redo", description = "Redo the most recent undo")]
    ...
    #[tool(
        name = "yomika.open_project",
        description = "Open or create a Yomika project directory"
    )]
...
pub fn mount(router: axum::Router, state: AppState) -> axum::Router {
    ...
    let factory = {
        ...
        move || -> Result<YomikaServer, std::io::Error> { Ok(YomikaServer::new(state.clone())) }
    };
    let service =
