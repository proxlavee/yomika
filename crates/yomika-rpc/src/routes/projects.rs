...
//! paths. A project's `id` is the `.ymkproj/` directory basename.
//!
//! - `GET    /projects` — list managed projects
//! - `POST   /projects` — create a new project (`{name}`), server allocates path
//! - `POST   /projects/import` — extract a `.ymk` archive into a fresh dir + open
...
use yomika_app::projects as project_dirs;
use yomika_core::{ImageRole, PageId, ProjectSummary};
...
async fn create_project(
    ...
    Ok(Json(yomika_app::app::project_summary(&session)))
}
...
pub struct OpenProjectRequest {
    /// `.ymkproj/` directory basename (no extension). Must exist under the
...
#[utoipa::path(
    ...
    request_body(content = inline(BinaryPayload), content_type = "application/zip"),
    responses((status = 200, body = ProjectSummary))
)]
async fn import_project(
    ...
    // Atomic-created dir must be removed so `import_ymk_bytes` can do its
    // own exists-check + populate.
    std::fs::remove_dir(dest.as_std_path())
        .map_err(|e| ApiError::internal(anyhow::Error::new(e)))?;

    let dest_c = dest.clone();
    tokio::task::spawn_blocking(move || yomika_app::archive::import_ymk_bytes(&body, &dest_c))
        .await
        .map_err(|e| ApiError::internal(anyhow::Error::new(e)))?
        .map_err(ApiError::internal)?;
...
pub enum ExportFormat {
    /// Whole project as a `.ymk` archive (always a single zip).
    Ymk,
    /// One `.psd` per page.
    Psd,
...
async fn export_current_project(
    ...
    match req.format {
        ExportFormat::Ymk => {
            ...
            let bytes =
                tokio::task::spawn_blocking(move || yomika_app::archive::export_ymk_bytes(&src))
                    .await
                    .map_err(|e| ApiError::internal(anyhow::Error::new(e)))?
                    .map_err(ApiError::internal)?;
            Ok(bytes_response(
...
async fn export_image_role(
    session: &std::sync::Arc<yomika_app::ProjectSession>,
    pages: Option<&[PageId]>,
    role: ImageRole,
    project_name: &str,
) -> ApiResult<Response> {
...
fn resolve_page_ids(
    session: &yomika_app::ProjectSession,
...
fn files_to_response(
    ...
    if files.len() == 1 {
        ...
        let content_type = match ext {
            ...
            "png" => "image/png",
            "ymk" => "application/octet-stream",
            _ => "application/octet-stream",
        };
        return Ok(bytes_response_with_filename(bytes, &fname, content_type));
    }
    let zip_bytes = yomika_app::archive::zip_files_to_bytes(&files).map_err(ApiError::internal)?;
