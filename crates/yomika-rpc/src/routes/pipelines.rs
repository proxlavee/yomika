...
pub struct StartPipelineRequest {
    ...
    /// Internal correlation used by the UI's debounced auto-render. When set,
    /// the request must be a single-page, renderer-only pipeline.
    #[serde(default)]
    pub auto_render_epoch: Option<u64>,
}
...
async fn start_pipeline(
    ...
    if req.auto_render_epoch.is_some() {
        let one_page = matches!(req.pages.as_deref(), Some([_]));
        let renderer_only = matches!(req.steps.as_slice(), [id]
            if pipeline::Registry::find(id)
                .is_ok_and(|info| info.produces.contains(&pipeline::Artifact::FinalRender)));
        if !one_page || !renderer_only {
            return Err(ApiError::bad_request(
                "autoRenderEpoch requires one page and one renderer step",
            ));
        }
    }
    let spec = PipelineSpec {
        ...
        options: PipelineRunOptions {
            ...
            auto_render_epoch: req.auto_render_epoch,
        },
    };
