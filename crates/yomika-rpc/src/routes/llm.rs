...
use yomika_core::{LlmCatalog, LlmLoadRequest, LlmState, LlmTargetKind};
...
async fn put_current_llm(
    ...
    let provider_config = match req.target.kind {
        LlmTargetKind::Provider => req.target.provider_id.as_deref().map(|pid| {
            yomika_app::llm::provider_config_from_settings(&app.config.load(), &app.runtime, pid)
        }),
...
async fn get_catalog(State(app): State<AppState>) -> ApiResult<Json<LlmCatalog>> {
    let catalog = yomika_app::llm::catalog(&app.config.load(), &app.runtime).await;
    Ok(Json(catalog))
}
