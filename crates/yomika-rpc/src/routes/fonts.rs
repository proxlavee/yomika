...
use yomika_core::{FontFaceInfo, GoogleFontCatalog};
...
async fn fetch_google_font(
    ...
    let (family, weight, style) = yomika_app::google_fonts::parse_variant_query(&family_query);
...
#[utoipa::path(
    ...
    responses((status = 200, body = inline(BinaryPayload), content_type = "font/ttf"))
)]
