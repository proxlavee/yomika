...
use yomika_core::{BlobRef, ImageRole, NodeKind, PageId, Scene};

use crate::AppState;
use crate::error::{ApiError, ApiResult};
use crate::routes::BinaryPayload;
...
struct WireSnapshot<'a> {
    epoch: u64,
    scene: &'a yomika_core::Scene,
}

#[utoipa::path(
    get,
    path = "/scene.bin",
    responses((status = 200, body = inline(BinaryPayload), content_type = "application/octet-stream"))
)]
...
/// `.ymkproj/cache/thumbs/<page_id>.webp`; generated on first request.
...
#[utoipa::path(
    ...
    responses((status = 200, body = inline(BinaryPayload), content_type = "image/webp"))
)]
