...
//!
//! The patterns here map `yomika-ml` / `yomika-llm` outputs (plain
...
/// Convert a scene `(Transform, TextData)` pair into a `yomika-ml` `TextRegion`
...
pub fn text_node_to_region(transform: &Transform, text: &TextData) -> yomika_ml::types::TextRegion {
    yomika_ml::types::TextRegion {
        x: transform.x,
        y: transform.y,
...
pub fn core_text_direction_to_ml(d: yomika_core::TextDirection) -> yomika_ml::types::TextDirection {
    match d {
        yomika_core::TextDirection::Horizontal => yomika_ml::types::TextDirection::Horizontal,
        yomika_core::TextDirection::Vertical => yomika_ml::types::TextDirection::Vertical,
    }
}
...
pub fn upsert_image_blob(
    ...
    if let Some((node_id, _)) = find_image_node(scene, page, role) {
        Op::UpdateNode {
            page,
            id: node_id,
            patch: yomika_core::NodePatch {
                data: Some(NodeDataPatch::Image(yomika_core::ImageDataPatch {
                    blob: Some(blob),
                    opacity: None,
                    name: None,
            ...
            prev: yomika_core::NodePatch::default(),
        }
    } else {
        let at = {
...
pub fn upsert_mask_blob(scene: &Scene, page: PageId, role: MaskRole, blob: BlobRef) -> Op {
    if let Some((node_id, _)) = find_mask_node(scene, page, role) {
        Op::UpdateNode {
            ...
            id: node_id,
            patch: yomika_core::NodePatch {
                data: Some(NodeDataPatch::Mask(yomika_core::MaskDataPatch {
                    blob: Some(blob),
                })),
                transform: None,
                visible: None,
...
/// Translate the `yomika-ml` `TextDirection` primitive into the scene-layer one.
pub fn ml_text_direction_to_core(d: yomika_ml::types::TextDirection) -> yomika_core::TextDirection {
    match d {
        yomika_ml::types::TextDirection::Horizontal => yomika_core::TextDirection::Horizontal,
        yomika_ml::types::TextDirection::Vertical => yomika_core::TextDirection::Vertical,
    }
}

/// Translate a `yomika-ml::TextRegion` (detector output) into a scene-layer
/// `(bbox, TextData)` pair ready for `new_text_node`.
pub fn text_region_to_pair(
    r: yomika_ml::types::TextRegion,
