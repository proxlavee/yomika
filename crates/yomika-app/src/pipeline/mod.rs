...
use yomika_core::{Op, PageId, PipelineStep};
...
pub async fn run(
    ...
    'pages: for (page_index, page_id) in pages.iter().enumerate() {
        for (seq, &i) in order.iter().enumerate() {
            ...
            if let Some(expected_epoch) = spec.options.auto_render_epoch
                && session.epoch() != expected_epoch
            {
                tracing::debug!(
                    ...
                    "skipped stale auto-render before loading its engine"
                );
                completed += (total_steps - seq) as u64;
                continue 'pages;
            }
            ...
            let apply_result = if let Some(expected_epoch) = spec.options.auto_render_epoch {
                match session.apply_auto_render(batch, expected_epoch, *page_id) {
                    Ok(Some(epoch)) => Ok(epoch),
                    Ok(None) => {
                        tracing::debug!(
                            page = %page_id,
                            expected_epoch,
                            "discarded stale auto-render output"
                        );
                        continue;
                    }
                    Err(err) => Err(err),
...
use yomika_core::{EngineCatalog, EngineCatalogEntry};
