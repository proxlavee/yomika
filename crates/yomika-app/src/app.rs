...
//! - `runtime` — ONNX session + device management (`yomika-runtime`)
...
use yomika_core::{AppEvent, DownloadProgress, JobSummary, LlmStateStatus};
use yomika_runtime::{ComputePolicy, RuntimeManager};
...
impl App {
    ...
    pub fn apply(&self, op: yomika_core::Op) -> Result<u64> {
        let session = self
            .current_session()
...
/// session's directory basename (the managed `<id>.ymkproj` form).
pub fn project_summary(session: &ProjectSession) -> yomika_core::ProjectSummary {
...
pub fn default_runtime() -> Result<Arc<RuntimeManager>> {
    let runtime = RuntimeManager::new(
        yomika_runtime::default_app_data_root(),
