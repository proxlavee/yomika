...
//!
//! Every `.ymkproj/` lives here. Clients address projects by `id` (the
//! directory basename without the `.ymkproj` extension). No path handling on
...
use yomika_core::ProjectSummary;

use crate::AppConfig;

pub const PROJECT_EXT: &str = "ymkproj";
...
/// Pick a fresh `{projects_dir}/{slug}.ymkproj` path for a new project with
...
/// List every `.ymkproj/` directory under the managed projects root. Reads
...
/// Produce an id for a `.ymkproj/` directory basename.
