//! A loaded project. One `ProjectSession` = one `.ymkproj/` directory.
//!
//! Holds:
...
//!
//! On-disk layout:
//!   `.ymkproj/project.toml`    — TOML-encoded `ProjectMeta`
//!   `.ymkproj/scene.bin`       — postcard-encoded `Snapshot { epoch, scene }`
//!   `.ymkproj/history.log`     — append-only `LogFrame { epoch, op }`
//!   `.ymkproj/blobs/ab/cdef…`  — content-addressed blobs
//!   `.ymkproj/.lock`           — fs4 exclusive lock (session lifetime)
...
impl ProjectSession {
    /// Open an existing `.ymkproj/` directory.
    ...
    /// Create a fresh `.ymkproj/` at `dir`, failing if it already exists.
    ...
    /// Apply an auto-render only if `expected_epoch` is still the latest edit
    /// on `page`, merging both into one undo entry.
    pub fn apply_auto_render(
        &self,
        op: Op,
        expected_epoch: u64,
        page: yomika_core::PageId,
    ) -> Result<Option<u64>> {
        let mut history = self.history.lock();
        let mut scene = self.scene.write();
        history.apply_auto_render(&mut scene, op, expected_epoch, page)
    }
