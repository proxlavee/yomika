...
//!
//! Blobs live at `.ymkproj/blobs/ab/cdef…` (hex blake3 hash, sharded by the
//! first two chars). Immutable: a blob with a given hash is always the same
//! bytes. An in-memory LRU decodes images on demand.
//!
//! `BlobRef` itself lives in `yomika-core::blob`; this module only provides
