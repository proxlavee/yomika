...
//!
//! ```text
//! cargo run --features bin -p yomika-app --bin pipeline -- \
//!     --input sample.png \
//!     --output-dir out/
//! ```
//!
...
#[derive(Parser, Debug)]
#[command(version, about = "Run the Yomika pipeline against a single image")]
struct Cli {
...
async fn run() -> Result<()> {
    ...
    if let Some(model_id) = cli.llm.as_deref() {
        ...
        app.llm
            .load_from_request(
                yomika_core::LlmLoadRequest {
                    target: yomika_core::LlmTarget {
                        kind: yomika_core::LlmTargetKind::Local,
    ...
    let progress_sink: yomika_app::pipeline::ProgressSink =
        Arc::new(|tick: yomika_app::pipeline::ProgressTick| {
    ...
    let spec = yomika_app::pipeline::PipelineSpec {
        scope: yomika_app::pipeline::Scope::Pages(vec![page_id]),
        steps,
        options: yomika_app::PipelineRunOptions {
    ...
    let warning_sink: yomika_app::pipeline::WarningSink =
        Arc::new(|tick: yomika_app::pipeline::WarningTick| {
            eprintln!(
    ...
    let result = yomika_app::pipeline::run(
        session.clone(),
        app.registry.clone(),
        app.runtime.clone(),
        app.cpu_only(),
...
async fn wait_for_llm_ready(app: &App) -> Result<()> {
    loop {
        ...
        match snap.status {
            yomika_core::LlmStateStatus::Ready => return Ok(()),
            yomika_core::LlmStateStatus::Failed => {
...
async fn synthesize_translations(app: &App, page: PageId) -> Result<()> {
    ...
    let mut ops = Vec::new();
    {
        ...
        for (id, node) in &page_data.nodes {
            ...
            {
                let patch = yomika_core::NodeDataPatch::Text(yomika_core::TextDataPatch {
