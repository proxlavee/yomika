...
// Similar role as the yolo-v8 ConvBlock (model.rs lines 74-112) but keeps BN explicit.
// refer: https://github.com/huggingface/candle/blob/95ea4538c3527f7cf001559281f45fe3ddaab359/candle-examples/examples/yolo-v8/model.rs#L74-L112
#[derive(Debug)]
struct ConvBnSiLu {
    conv: Conv2d,
    bn: BatchNorm,
...
// Matches the yolo-v8 Bottleneck layout reference (model.rs lines 114-148).
#[derive(Debug)]
struct Bottleneck {
    cv1: ConvBnSiLu,
    cv2: ConvBnSiLu,
...
// Parallels the yolo-v8 C2f block usage (model.rs lines 150-188) for the v5 C3 module.
#[derive(Debug)]
struct C3 {
    cv1: ConvBnSiLu,
    cv2: ConvBnSiLu,
...
// Mirrors the yolo-v8 SPPF block wiring (model.rs lines 190-231).
#[derive(Debug)]
struct Sppf {
    cv1: ConvBnSiLu,
    cv2: ConvBnSiLu,
...
// CSPDarknet53 backbone (paper naming), mirroring the yolo-v8 DarkNet split (model.rs lines 264-373) with v5 layer shapes.
#[derive(Debug)]
struct CspDarknet53 {
    l0: ConvBnSiLu,
    l1: ConvBnSiLu,
...
impl CspDarknet53 {
    ...
    fn forward(&self, xs: &Tensor) -> Result<(Tensor, Tensor, Tensor, Vec<Tensor>)> {
        // Returns P3/P4/P5-like features similar to DarkNet forward (model.rs lines 363-373).
...
// PANet neck (paper naming) following the fpn pattern from yolo-v8 (model.rs lines 376-464) with v5 tensors.
#[derive(Debug)]
struct PanetNeck {
    up: Upsample,
    l10: ConvBnSiLu,
...
// YOLOv3-style detection head; layout references yolo-v8 DetectionHead (model.rs lines 532-624).
#[derive(Debug)]
struct YoloV3Head {
    convs: [Conv2d; 3],
    anchors: Tensor,
