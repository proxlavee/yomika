...
fn meta_for_level(
    level: crate::sys::ggml_log_level,
) -> (&'static Metadata<'static>, &'static OverridableFields) {
    match level {
        crate::sys::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        crate::sys::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        crate::sys::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        crate::sys::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(crate::sys::ggml_log_level, String)>>,
...
impl State {
    ...
    fn generate_log(target: Module, level: crate::sys::ggml_log_level, text: &str) {
    ...
    pub(super) fn emit_non_cont_line(&self, level: crate::sys::ggml_log_level, text: &str) {
        ...
        match level {
            crate::sys::GGML_LOG_LEVEL_NONE => {
                // TODO: Support logging this to stdout directly via options?
                tracing::info!(no_log_level = true, text);
            }
            crate::sys::GGML_LOG_LEVEL_DEBUG
            | crate::sys::GGML_LOG_LEVEL_INFO
            | crate::sys::GGML_LOG_LEVEL_WARN
            | crate::sys::GGML_LOG_LEVEL_ERROR => Self::generate_log(self.module, level, text),
            crate::sys::GGML_LOG_LEVEL_CONT => unreachable!(),
            _ => {
                tracing::warn!(
                    level = level,
                    text = text,
    ...
    pub(super) fn is_enabled_for_level(&self, level: crate::sys::ggml_log_level) -> bool {
        // CONT logs do not need to check if they are enabled.
        let level = if level == crate::sys::GGML_LOG_LEVEL_CONT {
            self.previous_level
...
mod tests {
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(crate::sys::GGML_LOG_LEVEL_CONT, c"world".as_ptr(), log_ptr);
        logs_to_trace(crate::sys::GGML_LOG_LEVEL_CONT, c"\n".as_ptr(), log_ptr);
    }
    ...
    fn cont_enabled_log() {
        ...
        logs_to_trace(crate::sys::GGML_LOG_LEVEL_INFO, c"Hello ".as_ptr(), log_ptr);
        logs_to_trace(
            crate::sys::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
