...
use super::{PLAN_CACHE_CAPACITY, ShapeCache};
...
fn plan_cache() -> &'static Mutex<ShapeCache<PlanKey, Arc<CachedPlan>>> {
    static CACHE: OnceLock<Mutex<ShapeCache<PlanKey, Arc<CachedPlan>>>> = OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(ShapeCache::new(PLAN_CACHE_CAPACITY)))
}
...
/// Drop old plans only after the owning LaMa device has synchronized.
///
/// A checked-out `Arc` belongs to a concurrent forward and is not eligible;
/// the cache may briefly exceed its limit and will retry at the next forward
/// boundary.
pub(super) fn prune_plan_cache() {
    let removed = {
        let mut cache = plan_cache().lock().expect("cufft cache poisoned");
        cache.prune(|plan| Arc::strong_count(plan) == 1)
    };
    let removed_count = removed.len();
    drop(removed);
    if removed_count > 0 {
        tracing::debug!(removed = removed_count, "pruned cuFFT plan cache");
    }
}
