...
thread_local! {
    static FFT_PLANS: RefCell<ShapeCache<FftKey, FftPlan>> = RefCell::new(ShapeCache::new(PLAN_CACHE_CAPACITY));
...
pub(super) fn prune_plan_cache() {
    FFT_PLANS.with(|plans| {
        // MPS graph handles are retained. Any command that still needs one
        // owns its own Objective-C reference, so releasing the cache's clone
        // after a complete model forward is safe.
        let removed = plans.borrow_mut().prune(|_| true);
        if !removed.is_empty() {
            tracing::debug!(removed = removed.len(), "pruned MPS FFT plan cache");
        }
    });
}
