...
yomika_runtime::declare_hf_model_package!(
...
impl Lama {
    ...
    fn inference_model(&self, image: &RgbImage, mask: &GrayImage) -> Result<RgbImage> {
        // FFT plans are cached by exact crop shape. Prune only after tensors
        // have dropped and queued work has synchronized; immediate LRU
        // eviction could destroy a cuFFT plan still referenced by the GPU.
        let _fft_cleanup = FftPlanCacheCleanup::new(&self.device);
...
struct FftPlanCacheCleanup<'a> {
    device: &'a Device,
}

impl<'a> FftPlanCacheCleanup<'a> {
    fn new(device: &'a Device) -> Self {
        Self { device }
    }
}

impl Drop for FftPlanCacheCleanup<'_> {
    fn drop(&mut self) {
        if let Err(err) = self.device.synchronize() {
            tracing::warn!("failed to synchronize before pruning FFT plans: {err}");
            return;
        }
        fft::prune_plan_caches();
    }
}
