...
pub fn initialize(runtime: &RuntimeManager) -> Result<()> {
    let runtime_dir = runtime.llama_directory().context(
        "failed to resolve the llama runtime directory; call `yomika_runtime::prepare()` first",
    )?;

    if !runtime_dir.exists() {
        bail!(
            "runtime directory `{}` does not exist; call `yomika_runtime::prepare()` first",
            runtime_dir.display()
        );
    }
    ...
    if let Some(existing) = LIBRARIES.get() {
        ...
        bail!(
            "yomika-llm is already initialized with `{}` and cannot be reinitialized with `{}`",
    ...
    LIBRARIES
        .set(libraries)
        .map_err(|_| anyhow!("yomika-llm runtime libraries were initialized concurrently"))?;
...
fn load_and_bind<T>(
    ...
    let _ = path;
    #[cfg(target_os = "windows")]
    let library = yomika_runtime::load_library_by_name(name)
        .with_context(|| format!("failed to load `{name}`"))?;
    #[cfg(not(target_os = "windows"))]
    let library = yomika_runtime::load_library_by_path(path)
        .with_context(|| format!("failed to load `{name}`"))?;
...
fn libraries() -> &'static LoadedLibraries {
    LIBRARIES.get().expect(
        "yomika-llm runtime libraries are not initialized; call `yomika_runtime::prepare()` first",
    )
}
