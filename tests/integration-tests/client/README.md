# Rust API client for yomika-client
...
Put the package under your project folder in a directory named `yomika-client` and add the following to `Cargo.toml` under `[dependencies]`:

```
yomika-client = { path = "./yomika-client" }
```
