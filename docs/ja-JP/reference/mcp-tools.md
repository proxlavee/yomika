...
現在の実装は意図的に、プロジェクトのライフサイクル、履歴レイヤー、パイプラインジョブを中心とした小さく低レベルなサーフェスだけを公開しています。フィールド単位の細かな編集は、専用ツールではなく `yomika.apply` に `Op` ペイロードを渡して行います。
...
| ツール                  | 役割                                                       | パラメータ                                                                       |
| ----------------------- | ---------------------------------------------------------- | -------------------------------------------------------------------------------- |
| `yomika.apply`          | 有効なシーンに `Op` を適用する                             | `op` — JSON タグ付きの `Op` 値                                                   |
| `yomika.undo`           | 直近の op を取り消す                                       | なし                                                                             |
| `yomika.redo`           | 直近に取り消された op を再適用する                         | なし                                                                             |
| `yomika.open_project`   | Yomika プロジェクトディレクトリを開く、または作成する      | `path`、任意で `createName`                                                      |
| `yomika.close_project`  | 現在のプロジェクトを閉じる                                 | なし                                                                             |
| `yomika.start_pipeline` | パイプライン実行を開始し `jobId` を返す                    | `steps[]`、任意で `pages[]`、`targetLanguage`、`systemPrompt`、`defaultFont`     |
...
1. `yomika.open_project` — 管理対象のプロジェクトディレクトリを指定する
2. HTTP 経由で `GET /api/v1/scene.json` を読み、シーンを確認する
3. 次のいずれか:
    - 明示的な `Op` ペイロードで `yomika.apply` を呼び、絞り込んだ編集を行う
    - `yomika.start_pipeline` でエンドツーエンドのパイプラインを実行し、`GET /api/v1/events` を監視する
4. HTTP 経由で `POST /api/v1/projects/current/export` から書き出す
5. `yomika.close_project`

`yomika.undo` と `yomika.redo` は、間違った op を適用してしまい、逆 op を手で計算するより取り消した方が早いときに便利です。
