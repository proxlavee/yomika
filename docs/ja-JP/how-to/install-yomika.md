...
現在はビルド済みリリースを前提としていません。Windows、macOS、Linux 向けのデスクトップバイナリは、[ソースからビルドする](build-from-source.md) の手順で作成してください。今後公開される成果物は [Yomika releases ページ](https://github.com/proxlavee/yomika/releases) に表示されます。

## ローカルに何が入るのか

Yomika は local-first のアプリです。実際には、デスクトップバイナリだけがインストール対象ではありません。最初に本格的に起動したとき、ユーザーごとのローカルデータディレクトリも作成され、次のものが置かれます。
