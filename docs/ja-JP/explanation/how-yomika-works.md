...
Yomika は、漫画翻訳のためのページパイプラインを中心に設計されています。ユーザーから見た操作はシンプルですが、実装では joint detection、segmentation、OCR、inpainting、翻訳、レンダリングを意図的に別段階に分けています。
...
公開されているパイプライン段階としては、Yomika は次の順で動きます。
...
この分割によって、Yomika は「ページ上のどこにテキストがあるか」を決めるモデルと、「どのピクセルを実際に消すべきか」を決めるモデルを分けて使えます。
...
| Render | Yomika renderer | 最終的なローカライズ済みページまたは export |
...
そのため、1 つのモデルだけでは足りないことが多くなります。Yomika はまずテキストブロックと吹き出しを見つけ、次に切り出し領域へ OCR をかけ、segmentation mask を使ってクリーンアップし、その後で LLM に翻訳を依頼します。

## 実装の形

ソースコード上では、エンジンの登録とパイプライン実行は `crates/yomika-app/src/pipeline/engine.rs` にあり、既定エンジンの選択は `crates/yomika-app/src/config.rs` にあります。
