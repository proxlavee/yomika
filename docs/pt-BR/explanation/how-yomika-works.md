---
title: Como o Yomika Funciona
---

# Como o Yomika Funciona

O Yomika é construído em torno de uma pipeline por estágios para tradução de mangá. O editor apresenta essa pipeline como um fluxo de trabalho simples, mas a implementação mantém detection, segmentation, OCR, inpainting, tradução e renderização separados porque cada estágio produz dados diferentes e falha de formas diferentes.
...
No nível público da pipeline, o Yomika executa:
...
Essa divisão permite que o Yomika use um modelo para raciocinar sobre a estrutura da página e outro para decidir quais pixels exatos devem ser removidos.
...
| Render | renderer do Yomika | página localizada final ou exportação |
...
Por causa disso, um único modelo geralmente não é suficiente. O Yomika primeiro encontra blocos de texto e regiões de balão, depois executa OCR em regiões recortadas, em seguida usa um mask de segmentation para limpeza e só então pede a um LLM para traduzir o texto.

## A forma da implementação

Na árvore de código-fonte, o registro de engines e a execução da pipeline ficam em `crates/yomika-app/src/pipeline/engine.rs`, enquanto a seleção padrão de engine fica em `crates/yomika-app/src/config.rs`.
...
- quando você escolhe um provedor LLM remoto, o Yomika envia o texto do OCR para tradução, não a imagem completa da página
...
Por padrão, o Yomika executa:

- modelos de visão localmente
- LLMs locais localmente

Se você configurar um provedor LLM remoto, o Yomika envia apenas o texto do OCR selecionado para tradução a esse provedor.
