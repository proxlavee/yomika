...
This tutorial walks through the standard Yomika workflow for a single manga page: import, detect, recognize, translate, review, and export.

## Before you begin

- Install Yomika from the latest GitHub release
...
If you have not installed Yomika yet, start with [Install Yomika](../how-to/install-yomika.md).

## 1. Launch Yomika

Open the desktop application normally.

On the first run, Yomika may spend time initializing local runtime packages and downloading the default vision stack. This is expected and usually only happens once per machine or runtime update.

## 2. Import a page

Load your page image into the app.

At the moment, the documented import flow is image-based rather than project-file based. If you import a folder instead of a single file, Yomika recursively filters it down to supported image files.
...
Use Yomika's built-in vision pipeline to:
...
Under the hood, Yomika does not just run OCR on the full page. It first creates text blocks, crops those regions, and then runs OCR on the cropped areas.
...
Yomika can use OpenAI, Gemini, Claude, DeepSeek, and OpenAI-compatible endpoints such as LM Studio or OpenRouter.
...
- when you use a remote provider, Yomika sends OCR text for translation rather than the whole page image

## 5. Translate and review

Run translation on the page, then inspect the result carefully.

Yomika helps with text layout and vertical CJK rendering, but the final page still benefits from manual review. Focus on:

- names and terminology
- tone and character voice
- line breaks and bubble fit
- font choice and stroke readability
  Yomika's default stroke choice now auto-picks a black or white outline for contrast, but you can still override it manually when the page needs something else.
...
Yomika works best when you treat the pipeline as a fast first pass and then apply manual review where the page needs it.
