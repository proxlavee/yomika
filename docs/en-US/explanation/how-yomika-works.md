---
title: How Yomika Works
---

# How Yomika Works

Yomika is built around a staged page pipeline for manga translation. The editor presents that pipeline as a simple workflow, but the implementation keeps detection, segmentation, OCR, inpainting, translation, and rendering separate because each stage produces different data and fails in different ways.
...
At the public pipeline level, Yomika runs:
...
That split lets Yomika use one model to reason about page structure and another to decide which exact pixels should be removed.
...
| Render | Yomika renderer | final localized page or export |
...
Because of that, a single model is usually not enough. Yomika first finds text blocks and bubble regions, then runs OCR on cropped regions, then uses a segmentation mask for cleanup, and only after that asks an LLM to translate the text.

## The implementation shape

In the source tree, the engine registry and pipeline execution live in `crates/yomika-app/src/pipeline/engine.rs`, while default engine selection lives in `crates/yomika-app/src/config.rs`.
...
- when you choose a remote LLM provider, Yomika sends OCR text for translation, not the full page image
...
By default, Yomika runs:

- vision models locally
- local LLMs locally

If you configure a remote LLM provider, Yomika sends only the OCR text selected for translation to that provider.
