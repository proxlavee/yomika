...
当前实现刻意只暴露一个小而底层的接口面，聚焦在项目生命周期、历史层和管线任务上。细粒度编辑通过 `yomika.apply` 携带 `Op` 负载完成，而不是为每个字段单独提供工具。
...
| 工具                    | 作用                                              | 参数                                                                              |
| ----------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------- |
| `yomika.apply`          | 对当前场景应用一个 `Op`                            | `op`：带 JSON tag 的 `Op` 值                                                       |
| `yomika.undo`           | 撤销最近一次 op                                   | 无                                                                                |
| `yomika.redo`           | 重新应用最近一次撤销的 op                          | 无                                                                                |
| `yomika.open_project`   | 打开或创建一个 Yomika 项目目录                     | `path`，可选 `createName`                                                          |
| `yomika.close_project`  | 关闭当前项目                                      | 无                                                                                |
| `yomika.start_pipeline` | 启动一次管线运行；返回 `jobId`                     | `steps[]`，可选 `pages[]`、`targetLanguage`、`systemPrompt`、`defaultFont`         |
...
1. `yomika.open_project`：指向某个受管理的项目目录
2. 通过 HTTP 读取 `GET /api/v1/scene.json` 检查场景
3. 二选一：
    - 通过 `yomika.apply` 携带显式 `Op` 负载进行局部编辑，或者
    - 通过 `yomika.start_pipeline` 运行端到端管线，并监听 `GET /api/v1/events`
4. 通过 HTTP `POST /api/v1/projects/current/export` 导出
5. `yomika.close_project`

当某次 op 出错、想撤回而不是手算逆操作时，`yomika.undo` 与 `yomika.redo` 很有用。
