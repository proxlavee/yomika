...
description: 介绍 Yomika 文本渲染器的实现、为什么文本渲染本身很难，以及当前实现如何处理纵排 CJK 漫画文本。
---
...
一篇很值得参考的外部文章是 Aria Desires 的 [Text Rendering Hates You](https://faultlore.com/blah/text-hates-you/)。它的核心结论同样适用于 Yomika：文本渲染不是一条干净线性的流程，也不存在对所有情况都完美的答案。布局、整形、字体回退、栅格化和合成之间会互相牵制。

Yomika 并不想成为一个通用桌面排版引擎。它的目标是把漫画场景最常见、最关键的文本排版问题做好，尤其是纵排 CJK 气泡文本。
...
实现中，应用编排位于 `crates/yomika-app/src/renderer.rs`；布局、Shape、分段与光栅化代码位于 `crates/yomika-renderer/src/`。
...
Yomika 不会简单地把所有 CJK 文本都强制变成纵排。当前 `text/script.rs` 里的启发式规则是：
...
Yomika 会在 HarfBuzz 整形之前，把它转换成自上而下的整形方向。这样字体和整形引擎就有机会产出真正的纵排 advance 和纵排字形，而不是“先横排后旋转”的假象。
...
这正是字体用于纵排替换字形的标准 OpenType feature。这也是 Yomika 的纵排输出比“把横排文字旋转 90 度”看起来更像真的一个重要原因。

如果字体本身提供了正确的纵排替代字形，Yomika 就能用上；如果字体没有，那结果也只能退化到字体能提供的程度。
...
纵排 CJK 一旦对标点处理不好，观感会立刻变差。Yomika 对全角标点做了显式处理，并根据字体真实 bounds 对这些字形重新居中。
...
布局完成后，Yomika 会基于每个字形的 metrics 计算一套更紧致的 ink bounding box，然后平移 baseline，使真实墨迹区域从 `(0, 0)` 开始。
...
Yomika 并不打算实现所有高级文本排版功能。当前渲染器并没有完整覆盖下列能力：
...
最偷懒的纵排办法，就是先按横排排版，再把结果整体旋转。Yomika 避免这么做，因为常见失败模式太明显：
...
Yomika 把纵排处理前移到了整形与布局阶段，这就是它纵排 CJK 输出看起来更自然的核心架构决策。

## 值得读的外部参考

[Text Rendering Hates You](https://faultlore.com/blah/text-hates-you/) 很值得看，因为它从更通用的角度解释了渲染器为什么这么难做。Yomika 的技术栈和浏览器引擎不同，但其中的核心经验完全一致：
...
一句话总结：Yomika 的渲染器之所以这么小心，是因为文本渲染这件事本来就非常刁钻。
