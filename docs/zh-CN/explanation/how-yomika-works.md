...
Yomika 围绕一条漫画翻译页面管线构建。用户看到的流程很简单，但实现上会刻意把联合检测、分割、OCR、修复、翻译和渲染拆成独立阶段。
...
正因为如此，Yomika 才能让一个模型负责“文字在哪里”，另一个模型负责“哪些像素应该被擦掉”。
...
| Render | Yomika renderer | 最终本地化页面或导出内容 |
...
因此通常不能指望一个模型把所有事情都做好。Yomika 会先找出文本块和气泡区域，再对裁剪区域做 OCR，再用分割掩码做清理，最后才把文本交给 LLM 翻译。

## 实现结构

从源码结构看，引擎注册和管线执行在 `crates/yomika-app/src/pipeline/engine.rs`，默认引擎选择在 `crates/yomika-app/src/config.rs`。
...
- 当你选择远程 LLM 时，Yomika 发出去的是 OCR 文本，而不是整页图像
...
如果你配置了远程 LLM 提供商，Yomika 只会把待翻译文本发送给该提供商。
