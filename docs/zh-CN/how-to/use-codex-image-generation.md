...
Yomika 可以使用 Codex 进行端到端的 image-to-image 生成。这个流程会把源页面图像和提示词发送给 Codex，然后把生成出的图像保存为渲染后的页面结果。
...
这不同于 Yomika 的本地分阶段流水线。本地流水线会把检测、OCR、修复、翻译和渲染拆成独立步骤执行；Codex 流程会把页面图像发送到远程服务，并接收生成后的图像结果。
...
Codex 图像生成依赖 OpenAI 的上游服务。生成失败时，如果上游返回了响应文本和请求 ID，Yomika 会将其显示出来。临时故障有时可以通过重试解决。持续失败可能与账号访问权限、服务可用性，或后端对图像生成工具调用的支持状态有关。
