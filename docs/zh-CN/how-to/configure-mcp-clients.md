...
Yomika 通过本地 Streamable HTTP 暴露内置 MCP 服务器。本页说明如何把 MCP 客户端接到它上面，并给出 Antigravity、Claude Desktop 与 Claude Code 的详细配置步骤。

## Yomika 通过 MCP 暴露了什么

Yomika 的 MCP 服务器与桌面应用和 headless Web UI 使用的是同一个本地运行时。当前工具表面刻意保持精简，聚焦在项目生命周期、历史层和管线任务上：

- `yomika.open_project` / `yomika.close_project`
- `yomika.apply` / `yomika.undo` / `yomika.redo`
- `yomika.start_pipeline`

要进行更丰富的查询与编辑——例如场景快照、页面缩略图、blob 拉取、字体列表、LLM 控制、导出、配置——Agent 可以直接调用 Yomika 的 HTTP API（`http://127.0.0.1:<PORT>/api/v1`）。HTTP API 与 MCP 服务器共用同一个进程和状态，因此 Agent 可以在同一次工作流中自由混用两者。
...
- 当 MCP 客户端连接时，Yomika 进程必须保持运行
- Yomika 默认绑定到 `127.0.0.1`，因此以下示例都假定 MCP 客户端和 Yomika 在同一台机器上
- 默认本地配置下不需要额外鉴权头

## 2. 快速检查端点是否正常

在编辑任何客户端配置前，先确认 Yomika 确实已经运行在预期端口。
...
Antigravity 可以通过原始 MCP 配置直接指向 Yomika 的本地 URL。

### 步骤

1. 用 `--port 9999` 启动 Yomika。
2. 打开 Antigravity。
3. 打开编辑器 Agent 面板顶部的 `...` 菜单。
4. 点击 **Manage MCP Servers**。
5. 点击 **View raw config**。
6. 在 `mcpServers` 下新增一个 `yomika` 条目。
7. 保存配置。
...
如果你已经配置了其他 MCP 服务器，请把 `yomika` 加进去，不要直接覆盖整个 `mcpServers` 对象。

### 配好之后先试什么

先问几个简单问题：

- `你能使用哪些 Yomika MCP 工具？`
- `打开位于 C:\\projects\\my-manga.ymkproj 的 Yomika 项目。`

如果这一步通了，再尝试真实工作：

- `打开 C:\\projects\\my-manga.ymkproj 项目，并以 detect、ocr、llm-translate、aot-inpainting、yomika-renderer 这几个 step 启动管线。`
- `撤销 Yomika 中的最近一次编辑。`
- `把这个 Op 应用到第 <id> 页，新增一个文本块：{ ... }`

## Claude Desktop

Claude Desktop 当前本地 MCP 配置是基于命令的。由于 Yomika 暴露的是本地 HTTP MCP 端点，而不是打包成桌面扩展的插件，所以实际可行的配置方式是使用一个小型桥接进程，把 Claude Desktop 接到 `http://127.0.0.1:9999/mcp`。
...
- 如果你已经有其他 `mcpServers` 条目，请在保留原有配置的前提下增加 `yomika`
...
- `你能使用哪些 Yomika MCP 工具？`
- `打开位于 D:\\projects\\my-manga.ymkproj 的 Yomika 项目。`

然后再做真实工作：

- `在我刚才打开的项目上运行 Yomika 管线，step 包括 detect、ocr、llm-translate、aot-inpainting、yomika-renderer。`
- `用 Yomika 的 HTTP API（http://127.0.0.1:9999/api/v1/operations）查看管线状态。`
- `用 Yomika 的 HTTP API 把项目导出为 PSD。`

## Claude Code

如果你说的 “Claude” 指的是 Claude Code，那么连接 Yomika 的本地 `http://127.0.0.1` MCP 端点，最稳妥的方式同样是 stdio 桥接。
...
如果你已经在 Claude Desktop 里配置好了 Yomika，在支持的平台上也可以尝试导入：
...
- 让 Agent 列出可用的 Yomika MCP 工具
- 打开一个已存在的 Yomika 项目目录
...
- 启动 Yomika 时没有加 `--port`，却试图把客户端连到错误的端口
- 使用了 `http://127.0.0.1:9999/` 而不是 `http://127.0.0.1:9999/mcp`
- 添加完客户端配置后，把 Yomika 进程关掉了
- 直接覆盖整个客户端配置，而不是追加一个 `yomika` 条目
